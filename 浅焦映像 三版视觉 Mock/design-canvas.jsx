...
//
// On mobile devices the canvas automatically becomes a browse-only surface
// (touch pan/zoom, editing disabled). This needs NO authoring changes:
// never add custom touch handling or mobile-specific layouts.
/* END USAGE */
...
// Mirrors the host platform's isMobile() (UA regex + iPad maxTouchPoints);
// the product iframe is cross-origin, so the shared helper can't be imported.
// Deliberately NOT viewport-based: the desktop workbench shows canvases in a
// narrow preview iframe that a width breakpoint would misread as mobile.
// Evaluated once at load. Keep the regex in sync with deck-stage.js's copy.
...
if (typeof document !== 'undefined' && !document.getElementById('dc-styles')) {
  ...
  s.textContent = [
    ...
    // only suppresses native pan/zoom, not selection, so a drag
    // starting on selectable text never reaches the pan.
    ...
    // + pointercancel), so a pan could
    // never start there. Mobile canvas is browse-only: kill native
    ...
    // Section heads counter-scale via calc()'d font metrics (see DCSection),
    // NOT CSS zoom: zoom was only standardized in 2024 and legacy WebKit
    // builds (older iOS WKWebViews) mis-apply it — text renders unscaled
    // while layout inflates, tearing the title/subtitle apart at small
    // canvas scales. calc(px * var()) lengths are interop-safe everywhere
    // and keep the same in-flow layout semantics (box grows with the text,
    // space below stays reserved).
...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useEffect(() => {
    ...
    const zoomAt = (cx, cy, factor) => {
      ...
      // --dc-inv-zoom consumers (.dc-sectionhead's calc()'d font metrics,
      // each section's marginBottom) reflow on every scale change, vertically shifting the
    ...
    let pinchDist = 0;         // finger distance as of the last applied frame
    let pinchMid = null;       // last-applied midpoint
    let panApplied = null;     // pointer position as of the last applied pan frame
    ...
    const onTouchDown = (e) => {
      ...
      if (touches.size === 1) {
        ...
        mobFlushNow(); // apply any pending pan frame before rebasing as a pinch
        mobMode = 'pinch';
        const g = pinchGeom();
        pinchDist = g.d;
    ...
    // rAF-coalesced application: move handlers only record pointer
    // positions; geometry is applied at most once per frame. pointermove can
    // far outrun the render rate (120Hz touch sampling, fast pinches), and
    // running the full zoomAt/apply per event creates engine-heap garbage
    // faster than mobile WebKit reclaims it — which reads as unbounded
    // memory growth on device. Only the last position per frame can ever
    // be painted anyway.
    let mobRaf = 0;
    const mobFlushFrame = () => {
      mobRaf = 0;
      if (mobMode === 'pan') {
        ...
        if (!t) return;
        tf.current.x += t.x - panApplied.x;
        tf.current.y += t.y - panApplied.y;
        panApplied = { x: t.x, y: t.y };
        apply();
      } else if (mobMode === 'pinch' && touches.size >= 2) {
        const g = pinchGeom();
    ...
    const mobSchedule = () => { if (!mobRaf) mobRaf = requestAnimationFrame(mobFlushFrame); };
    const mobFlushNow = () => { if (mobRaf) { cancelAnimationFrame(mobRaf); mobFlushFrame(); } };
    ...
    const onTouchMove = (e) => {
      ...
      if (mobMode === 'pan' || mobMode === 'pinch') mobSchedule();
    };
    const onTouchUp = (e) => {
      mobFlushNow(); // apply the pending frame first so the gesture tail isn't lost
    ...
    const onHostMsg = (e) => {
      ...
      if (d && d.type === 'miaoda:canvas:set-zoom' && typeof d.scale === 'number') {
        ...
        // Re-announce the upgrade capability too — the host's readyGen reset
        // (fires on iframe 'load', which for image/font-heavy canvases lands
        // after the mount-time announce) drops prior state, so re-declare here.
        window.parent.postMessage({ type: 'miaoda:upgrade:available', kind: 'design-canvas' }, '*');
    ...
    window.parent.postMessage({ type: 'miaoda:canvas:present' }, '*');
    // Generic "upgrade" capability signal — announce that this artifact offers
    // an upgrade entry so the host can show/hide its upgrade button. Decoupled
    // from the canvas zoom-proxy protocol above; any product may post the same
    // message. Listener (onHostMsg) is already registered above → announce-after-
    // register order holds. No client-side behavior here.
    window.parent.postMessage({ type: 'miaoda:upgrade:available', kind: 'design-canvas' }, '*');
    lastPostedScale.current = undefined;
    apply();
    ...
    return () => {
      ...
      if (mobRaf) cancelAnimationFrame(mobRaf);
    };
  ...
  return (
    <div data-dc-section={sid}
      ...
      <div style={{ padding: '0 60px' }}>
        <div className="dc-sectionhead" style={{ paddingBottom: 'calc(36px * var(--dc-inv-zoom, 1))' }}>
          <DCEditable tag="div" value={sec.title ?? title}
            onChange={(v) => ctx && sid && ctx.patchSection(sid, { title: v })}
            style={{
              fontSize: 'calc(20px * var(--dc-inv-zoom, 1))', fontWeight: 600, color: DC.title,
              letterSpacing: 'calc(-0.4px * var(--dc-inv-zoom, 1))',
              marginBottom: 'calc(2px * var(--dc-inv-zoom, 1))',
              display: 'inline-block', lineHeight: 'calc(28px * var(--dc-inv-zoom, 1))',
            }} />
          {subtitle && <div style={{ fontSize: 'calc(14px * var(--dc-inv-zoom, 1))', color: DC.subtitle, lineHeight: 'calc(22px * var(--dc-inv-zoom, 1))' }}>{subtitle}</div>}
        </div>
      </div>
        ...
        <div className="dc-labelrow">
          ...
          <div className="dc-labeltext" onClick={onFocus} title="Click to focus">
            <DCEditable value={label} onChange={onRename} onClick={(e) => e.stopPropagation()}
              style={{ fontSize: 14, fontWeight: 400, color: DC.label, lineHeight: '22px' }} />
          </div>
        </div>
