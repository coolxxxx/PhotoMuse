...
- **开放接口**：`photomuseOpenApi` 网关（API Key 鉴权）对外提供目录 / 订单查询 / 收款码 / 运行配置四个 action，网站与其他小程序均可接入，见 `docs/开放接口接入指南.md`
- **管理后台**：OPENID 白名单 + 管理口令双因子登录，支持审核、重拍、标记支付、上传预览网格、派单（`ai_studio_jobs`）、交付、收款设置、运行时配置，全链路审计日志
